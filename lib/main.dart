...
class _RenderLauncherScreenState extends State<RenderLauncherScreen> {
  ...
  Future<void> _logStreamLocatorSnapshot() async {
    final locator = _lastLocator;
    _log(
      'Stream locator snapshot: ${locator == null ? 'none captured yet' : _locatorSummary(locator)}. '
      'Direct getCurrentLocator() is intentionally not called because this Flureadium build does not expose it.',
    );
  }

  Future<void> _skipToNextWithBoundaryDiagnostics() async {
    final before = _lastLocator;
    _log('Boundary test: skipToNext() requested from ${_boundarySummary(before)}.');
    await _flureadium.skipToNext();
    await Future<void>.delayed(const Duration(milliseconds: 700));
    _log('Boundary test: after skipToNext() stream locator is ${_boundarySummary(_lastLocator)}.');
  }

  Future<void> _skipToPreviousWithBoundaryDiagnostics() async {
    final before = _lastLocator;
    _log('Boundary test: skipToPrevious() requested from ${_boundarySummary(before)}.');
    await _flureadium.skipToPrevious();
    await Future<void>.delayed(const Duration(milliseconds: 700));
    _log('Boundary test: after skipToPrevious() stream locator is ${_boundarySummary(_lastLocator)}.');
  }

  Future<void> _jumpToAdjacentReadingOrder({required bool next}) async {
    final publication = _publication;
    final locator = _lastLocator;
    if (publication == null) {
      _log('Adjacent readingOrder jump skipped: no publication is open.');
      return;
    }
    if (locator == null) {
      _log('Adjacent readingOrder jump skipped: no stream locator captured yet.');
      return;
    }

    final currentIndex = _readingOrderIndexFor(locator.href);
    if (currentIndex == null) {
      _log(
        'Adjacent readingOrder jump skipped: locator href ${locator.href} was not found in readingOrder.',
      );
      return;
    }

    final targetIndex = next ? currentIndex + 1 : currentIndex - 1;
    if (targetIndex < 0 || targetIndex >= publication.readingOrder.length) {
      _log(
        'Adjacent readingOrder jump skipped: target index $targetIndex is outside readingOrder length ${publication.readingOrder.length}.',
      );
      return;
    }

    final target = publication.readingOrder[targetIndex];
    _log(
      'Adjacent readingOrder jump: ${next ? 'next' : 'previous'} from index $currentIndex '
      'to $targetIndex (${target.title ?? target.href}).',
    );
    await _goToTocLink(publication: publication, link: target);
    await Future<void>.delayed(const Duration(milliseconds: 700));
    _log('Adjacent readingOrder jump result: ${_boundarySummary(_lastLocator)}.');
  }

  String _boundarySummary(Locator? locator) {
    if (locator == null) return 'none';
    final index = _readingOrderIndexFor(locator.href);
    final indexText = index == null ? 'readingOrder=?' : 'readingOrder=$index';
    return '${_locatorSummary(locator)} · $indexText';
  }

  int? _readingOrderIndexFor(String href) {
    final publication = _publication;
    if (publication == null) return null;
    final normalizedLocatorHref = _normalizeHrefForCompare(href);
    for (var index = 0; index < publication.readingOrder.length; index += 1) {
      final link = publication.readingOrder[index];
      if (_normalizeHrefForCompare(link.href) == normalizedLocatorHref) return index;
    }
    return null;
  }

  String _normalizeHrefForCompare(String href) {
                  ...
                  FilledButton.icon(
                    ...
                    onPressed: () async {
                      try {
                        ...
                        _log('ttsEnable() completed with locator=${_lastLocator == null ? 'null' : _locatorSummary(_lastLocator!)}.');
                        await _flureadium.setDecorationStyle(
                          ReaderDecorationStyle(
                            style: DecorationStyle.highlight,
                            tint: const Color(0x55FFEB3B),
            ...
            children: [
              ...
              _buildLabSection(
                ...
                children: [
                  ...
                  _labButton(
                    'skip previous chapter + log',
                    Icons.first_page,
                    publication == null ? null : _skipToPreviousWithBoundaryDiagnostics,
                  ),
                  _labButton(
                    'skip next chapter + log',
                    Icons.last_page,
                    publication == null ? null : _skipToNextWithBoundaryDiagnostics,
                  ),
                  _labButton(
                    'jump previous readingOrder',
                    Icons.keyboard_double_arrow_up_outlined,
                    publication == null
                        ? null
                        : () => _jumpToAdjacentReadingOrder(next: false),
                  ),
                  _labButton(
                    'jump next readingOrder',
                    Icons.keyboard_double_arrow_down_outlined,
                    publication == null
                        ? null
                        : () => _jumpToAdjacentReadingOrder(next: true),
                  ),
                ],
              ),
              _buildLabSection(
              ...
              _buildLabSection(
                ...
                children: [
                  _labButton(
                    'Show stream locator',
                    Icons.my_location_outlined,
                    publication == null ? null : _logStreamLocatorSnapshot,
                  ),
                  _labButton(
                    'Apply saved decorations',
                    Icons.format_color_fill_outlined,
              ...
              Card(
                child: Padding(
                  padding: const EdgeInsets.all(12),
                  child: Text(
                    'Scroll-mode test rule: do not add Flutter drag bridges here. Enable vertical scroll, test native vertical movement, then use the boundary buttons to prove whether Flureadium can cross spine/chapter boundaries through skipToNext/skipToPrevious or explicit readingOrder jumps.',
                    style: Theme.of(context).textTheme.bodyMedium,
                  ),
                ),
              ),
